...
namespace UI {
    ...
    public class AtlasImageEditor : ImageEditor {
        ...
        void ResetAtlasSpriteNames() {
            ...
            if (newAtlas) {
                atlasSpriteNames = GetAtlasSpriteNames(newAtlas);
            }
        }
        /// <summary>
        /// SpriteAtlasからSprite名リストを取り出し
        /// </summary>
        public static string[] GetAtlasSpriteNames(SpriteAtlas atlas) {
            var list = GetAllSprite(atlas).Select(x => x.name.Replace("(Clone)", "")).ToArray();
            // ソート
            Array.Sort(list, StringComparer.CurrentCulture);
            return list;
        }
